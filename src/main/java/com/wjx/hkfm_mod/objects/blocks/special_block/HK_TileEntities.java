...
public class HK_TileEntities {
    ...
    public static class TileEntity_mt_b_injector extends TileEntity implements IInventory,ITickable {
        ...
        private ItemStack injecting = ItemStack.EMPTY;
        ...
        @Override
        public String getName() {
            return this.hasCustomName() ? this.customName : "container.mt_b_injector";
        }
        ...
        public boolean canInjected(){
            if(this.inventory.get(0).isEmpty()) return false;
            else {
                ItemStack result = HK_machines_recipes.mtInjector_Recipes.getInstance().getMtInjectorResult(this.inventory.get(0), this.inventory.get(1));
                if (result.isEmpty())return false;
                else {
        ...
        public void update() {
            ...
            if((cookTime>0)){
                ...
                if (cookTime == totalCookTime){
                    ...
                    else insertItem(2,injecting,false);

                    injecting = ItemStack.EMPTY;
                    cookTime = 0;
                    return;
                ...
                if (this.canInjected() && cookTime == 0){
                    ItemStack output = HK_machines_recipes.mtInjector_Recipes.getInstance().getMtInjectorResult(input1,input2);
                    if(!output.isEmpty()){
                        injecting = output;
                        cookTime++;
                        input1.shrink(1);
                        input2.shrink(1);
                        this.inventory.set(0,input1);
